...
import { randomNumbers, makeStorage, makeCrypto } from './protocolTestHelpers';
import { PrivateProfile, PublicProfile } from './models/Profile';
import { PrivateIdentity } from './models/Identity';
import { createDeterministicRandomGenerator } from './unsecureRandom';
import { makePostId } from './postHelpers';
...
import { generateSecureIdentity } from './identityHelpers';
...
const executeCommand = (command: GroupCommand, context: GroupContext, address: HexString) => {
    ...
    switch (command.type) {
        ...
        case 'remove-post': {
            ...
            console.log('executeCommand', command.type, {posts: context.posts});
            return;
        }
        case 'remove-member': {
...
export const execute = async (
    ...
    const generateIdentity = () => generateSecureIdentity(generateAsyncDeterministicRandom);
